...
function mockEntry(file: string, isNew = false, isDeleted = false): DiffEntry {
  return { file, content: "", isNew, isDeleted };
}
...
describe("buildFileTree", () => {
  ...
  test("sets correct status for new vs modified files", () => {
    const entries = [
      mockEntry("new.ts", true),
      mockEntry("modified.ts", false),
    ];
    const tree = buildFileTree(entries);
  ...
  test("sets correct status for deleted files", () => {
    const entries = [
      mockEntry("deleted.ts", false, true),
      mockEntry("modified.ts", false, false),
    ];
    const tree = buildFileTree(entries);

    expect(tree[0]!.status).toBe("D"); // deleted.ts
    expect(tree[1]!.status).toBe("M"); // modified.ts
  });
});
