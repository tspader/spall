...
export async function getDiffEntries(repoPath: string): Promise<DiffEntry[]> {
  ...
  // Detect deleted files (in HEAD but not in working tree)
  const deletedResult =
  ...
  for (const file of trackedFiles) {
    ...
    entries.push({ file, content, isNew: false, isDeleted });
  }
...
@@ -0,0 +1,${lines.length} @@
${diffLines.join("\n")}`;
    entries.push({ file, content, isNew: true, isDeleted: false });
  }
...
/**
 * Get a hash representing the current git diff state.
 * Used for polling to detect changes without fetching full diff content.
 */
export async function getDiffHash(repoPath: string): Promise<number> {
  // Get tracked changes summary
  const trackedResult =
    await $`git -C ${repoPath} diff HEAD --name-only --relative`.quiet();
  // Get untracked files
  const untrackedResult =
  ...
  // Combine and hash
  const combined =
    trackedResult.stdout.toString() + "\0" + untrackedResult.stdout.toString();
