/**
 * Selection state for navigating diff lines/blocks
 */
...
export interface SelectionState {
  ...
  selection: Selection
  /** Whether we're in line-by-line mode (vs block mode) */
  lineMode: boolean
...
/**
 * Toggle between line mode and block mode
 */
 ...
 */
export function moveDown(state: SelectionState, shift: boolean): SelectionState {
...
/**
 * Set selection to a specific block (used in block mode)
 */
export function setBlockSelection(
  ...
  return {
    ...state,
    selection: {
      start: Math.max(0, blockStart),
      end: Math.min(blockEnd, state.totalLines - 1),
    },
  }
}
 ...
 */
export function resetSelection(state: SelectionState, line: number): SelectionState {
