...
import { type LineSelection, getLineSelectionsForFile } from "../lib/line-selection"
...
function DiffIndicator(props: DiffIndicatorProps) {
  ...
  const lineColors = createMemo(() => {
    ...
    if (props.lineMode()) {
      ...
      const fileLineSelections = getLineSelectionsForFile(props.lineSelections(), fileIdx)
      ...
      for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
        ...
        const isSelected = isHunkSelected(props.selectedHunks(), fileIdx, blockIndex)
        const isFocused = props.focused() && blockIndex === props.selectedBlockIndex()
    ...
    <box flexDirection="column" width={1} flexShrink={0}>
      <For each={lineIndices()}>
        {(lineNum) => {
          ...
          return (
            <text height={1} fg={color()}>{color() ? "▌" : " "}</text>
          )
        }}
      </For>
    </box>
